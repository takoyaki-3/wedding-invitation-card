...
`public/photo/` に入れた JPG・PNG・WebP・AVIF を、ファイル名順に招待状冒頭のスライドショーで表示します。5秒ごとに自動で切り替わり、前後の移動・一時停止もできます。動きを減らす設定の端末では停止状態で始まります。写真がない場合はこのセクションを表示しません。写真の追加・削除後は開発サーバーの再起動、本番は再ビルド・再デプロイが必要です。写真はGit管理対象外ですが、デプロイ時には公開サイトに同梱されます。
