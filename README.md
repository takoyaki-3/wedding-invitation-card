...
紙の招待状は `npm run invitation:pdf` でA4のPDFとして生成します。`npm run dev` と `npm run build` でも自動生成します。ページと同じ環境設定・ロゴを使い、親族用と友人用で集合時間・場所を分けています。設定変更後は再生成してください。生成先の `public/invitation/*.pdf` はGit管理対象外で、本番ビルドには同梱されます。WindowsではMicrosoft Edge、それ以外ではPlaywrightのChromium（`npx playwright install chromium`）と日本語フォントが必要です。

テーブルの写真は `public/` に同梱したイメージ写真です。Google Fontsと会場のGoogleマップの表示には外部ネットワーク接続を使います。
